...
class ModelSelection:
    ...
    def load_data(self):
        try:
            ...
            df = pd.read_csv(self.data_path) # 100% data is there 
    ...
    def log_confusion_matrix(self,y_true,y_pred,step,model_name):
        ...
        plt.title(f"Confusion matric for {model_name}")
    ...
    def train_and_evaluate(self,X_train,X_test,y_train,y_test):
        try:
            ...
            for idx,(name,model) in enumerate(self.models.items()):
                ...
                self.writer.add_text('Model Details',f"Name :{name}" f"Metrics: Accuracy:{accuracy} , Precision:{precision},Recall:{recall},f1_score:{f1}")
    ...
    def run(self):
        try:
            logger.info("Starting the Model selection Pipeline")
            X,y= self.load_data()
